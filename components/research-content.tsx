export function ResearchContent() {
  return (
    <div className="max-w-3xl mx-auto prose prose-gray">
      <h2 className="font-display text-2xl font-bold mb-4">Smart Money vs Dumb Money — A Clearer Look</h2>
      <p>
        In trading, there are two often-used terms:
        <strong> Smart Money</strong> and <strong>Dumb Money</strong>. But this isn't about intelligence — it's about
        strategy, structure, and outcomes.
      </p>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">🤔 Why "Smart" and "Dumb"?</h3>
      <p>
        <strong>Smart Money</strong> typically refers to institutional investors — hedge funds, asset managers, banks —
        who use informed decision-making, economic data, risk analysis, and strategic planning to guide their trades.
      </p>
      <p>
        <strong>Dumb Money</strong> is often the nickname for retail traders — everyday individuals who often trade
        emotionally, without a structured plan, or based on hype and social media trends.
      </p>
      <p>The labels aren't insults. They describe how decisions are made — and how money behaves in the market.</p>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">🎯 Expectations & Targets</h3>
      <p>
        <strong>Smart Money</strong> sets realistic, long-term goals:
      </p>
      <ul>
        <li>Capital preservation</li>
        <li>Risk-adjusted returns</li>
        <li>Portfolio growth over time</li>
      </ul>

      <p>
        <strong>Dumb Money</strong> often chases:
      </p>
      <ul>
        <li>Quick wins</li>
        <li>"Home-run" trades</li>
        <li>Unrealistic profit expectations (e.g., turning $100 into $10,000 in a week)</li>
      </ul>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">🛠 Decision-Making Framework</h3>
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse border border-gray-200 my-4">
          <thead>
            <tr className="bg-gray-50">
              <th className="border border-gray-200 px-4 py-2 text-left">Category</th>
              <th className="border border-gray-200 px-4 py-2 text-left">Smart Money</th>
              <th className="border border-gray-200 px-4 py-2 text-left">Dumb Money</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="border border-gray-200 px-4 py-2">Research Approach</td>
              <td className="border border-gray-200 px-4 py-2">Deep fundamental & technical analysis</td>
              <td className="border border-gray-200 px-4 py-2">Surface-level info or gut feeling</td>
            </tr>
            <tr>
              <td className="border border-gray-200 px-4 py-2">Risk Management</td>
              <td className="border border-gray-200 px-4 py-2">Controlled, calculated</td>
              <td className="border border-gray-200 px-4 py-2">Often ignored or misunderstood</td>
            </tr>
            <tr>
              <td className="border border-gray-200 px-4 py-2">Trade Planning</td>
              <td className="border border-gray-200 px-4 py-2">Pre-planned entries, exits, scenarios</td>
              <td className="border border-gray-200 px-4 py-2">On-the-fly decisions</td>
            </tr>
            <tr>
              <td className="border border-gray-200 px-4 py-2">Emotional Control</td>
              <td className="border border-gray-200 px-4 py-2">High</td>
              <td className="border border-gray-200 px-4 py-2">Low (FOMO, revenge trades, etc.)</td>
            </tr>
            <tr>
              <td className="border border-gray-200 px-4 py-2">Capital Focus</td>
              <td className="border border-gray-200 px-4 py-2">Growth and preservation</td>
              <td className="border border-gray-200 px-4 py-2">High risk → often leads to capital loss</td>
            </tr>
          </tbody>
        </table>
      </div>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">📊 Is Smart Money Technical or Fundamental?</h3>
      <p>It's not either/or — Smart Money uses both:</p>
      <ul>
        <li>Fundamental analysis for long-term investment value</li>
        <li>Technical analysis for timing and entry/exit efficiency</li>
      </ul>

      <p>Dumb Money, in contrast, may:</p>
      <ul>
        <li>Copy technical setups without context</li>
        <li>Follow influencers blindly</li>
        <li>Trade based on emotion, not evidence</li>
      </ul>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">📈 Results: What Do They Actually Earn?</h3>
      <p>
        <strong>Smart Money</strong> targets consistent, compounding returns (e.g., 8–20% yearly across portfolios)
      </p>
      <p>
        <strong>Dumb Money</strong> often seeks high returns fast, but:
      </p>
      <ul>
        <li>Lacks capital protection</li>
        <li>Suffers frequent drawdowns or full account losses</li>
      </ul>

      <h3 className="font-display text-xl font-semibold mt-6 mb-3">🔁 The Bigger Picture</h3>
      <p>In simple terms:</p>
      <ul>
        <li>
          <strong>Smart Money</strong> builds wealth.
        </li>
        <li>
          <strong>Dumb Money</strong> often burns it.
        </li>
      </ul>

      <p className="text-lg font-semibold mt-6">
        At Qantora, we don't shame "dumb money." We educate, equip, and empower everyday traders to make smart moves —
        turning guesswork into strategy and confusion into confidence.
      </p>

      <p>
        It's not about how much money you start with. It's about how you treat your money when it enters the market.
      </p>

      <p className="text-lg font-semibold">Qantora is here to shift that mindset — and help you trade smarter.</p>
    </div>
  )
}
